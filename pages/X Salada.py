...
with st.container():
    st.subheader(f"Resultado Total: R$ {total_x_salada:,.2f}")
    st.markdown("# Resultado por Ativo")
    fig = px.bar(df3_xsalada_total, x='codigo', y='liquido', color='codigo', labels={'codigo': '', 'liquido': 'Resultado'})
    st.plotly_chart(fig, use_container_width=True)
    st.markdown("""___""")
    
with st.container():
    st.markdown("# Estatísticas")
    st.dataframe(df3_combinado_xsalada, use_container_width=True)
    st.markdown("""___""")