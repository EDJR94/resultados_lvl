...
total_sala_ab_acoes = df3_abertura_acoes['liquido'].sum()
...
total_leilao = df3_hora_leilao['liquido'].sum()
...
total_x_salada = df3_xsalada['liquido'].sum()
...
#Gráfico acumulado
df_total_dia = df3_mes.loc[:,['data','liquido']].groupby('data').sum().reset_index()
...
with st.container():
    st.subheader(f"Resultado Total: R$ {round(df3_abertura_acoes['liquido'].sum(),2):,.2f}")
    st.markdown("# Top 10 Melhores Ações")
    df3_combinado_ab_acoes.sort_values(by='liquido', ascending=False)
    st.dataframe(df3_combinado_ab_acoes.head(10), use_container_width=True)
    st.markdown("""___""")
    
with st.container():
    st.markdown("# Top 10 Piores Ações")
    df3_combinado_ab_acoes.sort_values(by='liquido', ascending=False)
    st.dataframe(df3_combinado_ab_acoes.tail(10), use_container_width=True)
    st.markdown("""___""")
    
with st.container():
    st.markdown("# Geral")
    st.dataframe(df3_combinado_ab_acoes, use_container_width=True)
    st.markdown("""___""")